...
#define MAXFILESIZ 5242880
#define SPEED_MEASURE_DELAY 5 // The amount of seconds to wait before calculating the download speed
#define MIN_DOWN_SPEED 100 // The minimum speed (in bytes/sec) that needs to be maintained before closing the connection
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	// Evil mode: Write an extremely long filename to a peer in order to cause a buffer overflow
	if(evil_mode) {
		char malicious_filename[FILENAMESIZ * 10];
		int i;
		for(i = 0; i < FILENAMESIZ * 10; i++) 
			malicious_filename[i] = 'x';
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	time_t time_write_start;
	time(&time_write_start);
	...
	while (1) {
		...
		time_t time_last_write;
		time(&time_last_write);
		double time_elapsed = difftime(time_last_write, time_write_start);
		if((int) time_elapsed > SPEED_MEASURE_DELAY) {
			if(t->total_written / (int) time_elapsed < MIN_DOWN_SPEED) {
				error("* Peer is very slow so we are disconnecting");
				goto try_again;
			}
		}
		
		if(t->total_written >= MAXFILESIZ) {
			error("* Maximum file size limit has been reached, ending download");
			goto try_again;
		}
...
static void task_upload(task_t *t)
{
	...
	char *last_slash; 
	last_slash = strrchr(t->filename, '/');
	if(last_slash) {
		int last_slash_location = last_slash - t->filename;
		char *attempted_dir_name = malloc((size_t) last_slash_location);
		memcpy(attempted_dir_name, t->filename, (size_t) last_slash_location);
		attempted_dir_name[last_slash_location] = '\0';
		
		long dir_size = pathconf(".", _PC_PATH_MAX);
		char *allowed_dir_name;
		allowed_dir_name = malloc((size_t) dir_size);
		allowed_dir_name = getcwd(allowed_dir_name, (size_t) dir_size);
		
		if(strcmp(attempted_dir_name, allowed_dir_name) != 0) {
			error("* You do not have access to: %s but you do have access to: %s", attempted_dir_name, allowed_dir_name);
			goto exit;
		}
	}
	...
	if(evil_mode) {
		...
		char* randomData = malloc(1048576);
		read(devRandom, randomData, 1048576);
		while(1) {
			osp2p_writef(t->peer_fd, randomData);
		}
	}
	else {
		while (1) {
